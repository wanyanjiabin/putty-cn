...
echo "\e[33m |                                                       |\e[0m"
echo "\e[33m |   最后修改: 2023/03/07 01:23:04                       |\e[0m"
...
perl -i -pe 's#\Q"ChaCha20 (SSH-2 only)"#"ChaCha20 (仅限 SSH-2)"#g' $file_name
perl -i -pe 's#\Q"AES (SSH-2 only)"#"AES      (仅限 SSH-2)"#g' $file_name
perl -i -pe 's#\Q"AES-GCM (SSH-2 only)"#"AES-GCM  (仅限 SSH-2)"#g' $file_name
perl -i -pe 's#\Q"Arcfour (SSH-2 only)"#"Arcfour  (仅限 SSH-2)"#g' $file_name
...
perl -i -pe 's#\Q"Remote ports do the same (SSH-2 only)"#"远程端口接受连接(仅限 SSH-2)(P)"#g' $file_name
...
perl -i -pe 's#\Q"Command to send to proxy (for some types)"#"发送给代理的命令(对于某些类型)"#g' $file_name
perl -i -pe 's#\Q"HTTP CONNECT"#"HTTP 连接"#g' $file_name
perl -i -pe 's#\Q"SSH to proxy and use port forwarding"#"SSH 代理并使用端口转发"#g' $file_name
perl -i -pe 's#\Q"SSH to proxy and execute a command"#"SSH 代理并执行命令"#g' $file_name
perl -i -pe 's#\Q"SSH to proxy and invoke a subsystem"#"SSH 代理并调用子系统"#g' $file_name
perl -i -pe 's#\Q"Local (run a subprogram to connect)"#"本地(运行子程序连接)"#g' $file_name
perl -i -pe 's#\Q"'\''Telnet'\'' (send an ad-hoc command)"#"Telnet(发送临时命令)"#g' $file_name
perl -i -pe 's#\Q"Configure trusted certification authorities"#"配置受信任的证书颁发机构"#g' $file_name
perl -i -pe 's#\Q"Configure host CAs"#"配置主机 CAs"#g' $file_name
perl -i -pe 's#\Q"Diffie-Hellman group 1 (1024-bit)"#"Diffie-Hellman group 1 (1024位)"#g' $file_name
perl -i -pe 's#\Q"Diffie-Hellman group 14 (2048-bit)"#"Diffie-Hellman group 14 (2048位)"#g' $file_name
perl -i -pe 's#\Q"Diffie-Hellman group 15 (3072-bit)"#"Diffie-Hellman group 15 (3072位)"#g' $file_name
perl -i -pe 's#\Q"Diffie-Hellman group 16 (4096-bit)"#"Diffie-Hellman group 16 (4096位)"#g' $file_name
...
perl -i -pe 's#\Q"Select certificate file"#"选择证书文件"#g' $file_name
perl -i -pe 's#\Q"Plugin to provide authentication responses"#"提供身份验证响应的插件"#g' $file_name
perl -i -pe 's#\Q"Plugin command to run"#"要运行的插件命令"#g' $file_name
perl -i -pe 's#\Q"Chokes on PuTTY'\''s full KEXINIT"#"PuTTY 的完整 KEXINIT 阻塞"#g' $file_name
...
# ssh/ca-config.c
file_name="ssh/ca-config.c"
echo [56%]开始汉化 ssh/ca-config.c
perl -i -pe 's#\Q"Cannot decode key: %s"#"无法解码密钥: %s"#g' $file_name
perl -i -pe 's#\Q"Invalid key (no key type)"#"无效密钥 (无密钥类型)"#g' $file_name
perl -i -pe 's#\Q"Unrecognised key type '\''%.*s'\''"#"无法识别的密钥类型 '\''%.*s'\''"#g' $file_name
perl -i -pe 's#\Q"CA key may not be a certificate (type is '\''%.*s'\'')"#"CA密钥可能不是证书 (类型为 '\''%.*s'\'')"#g' $file_name
perl -i -pe 's#\Q"Invalid '\''%.*s'\'' key data"#"无效的 '\''%.*s'\'' 键数据"#g' $file_name
perl -i -pe 's#\Q"Unable to load host CA record '\''%s'\''"#"无法加载主机CA记录'%s'"#g' $file_name
perl -i -pe 's#\Q"No validity expression configured "#"没有为此密钥配置有效性表达式"#g' $file_name
perl -i -pe 's#\Q"for this key"#""#g' $file_name
perl -i -pe 's#\Q"Error in expression: %s"#"表达式错误: %s"#g' $file_name
perl -i -pe 's#\Q"No valid CA public key entered"#"没有输入有效的 CA 公钥"#g' $file_name
perl -i -pe 's#\Q"Unable to load public key from '\''%s'\'': %s"#"无法从“%s”加载公钥：%s"#g' $file_name
perl -i -pe 's#\Q"Load, save or delete a host CA record"#"加载、保存或删除主机 CA 记录"#g' $file_name
perl -i -pe 's#\Q"Name for this CA (shown in log messages)"#"此 CA 的名称（显示在日志消息中）"#g' $file_name
perl -i -pe 's#\Q"Load"#"加载"#g' $file_name
perl -i -pe 's#\Q"Save"#"保存"#g' $file_name
perl -i -pe 's#\Q"Delete"#"删除"#g' $file_name
perl -i -pe 's#\Q"Public key for this CA record"#"此 CA 记录的公钥"#g' $file_name
perl -i -pe 's#\Q"Public key of certification authority"#"证书颁发机构的公钥"#g' $file_name
perl -i -pe 's#\Q"Read from file"#"从文件中读取"#g' $file_name
perl -i -pe 's#\Q"Select public key file of certification authority"#"选择证书颁发机构的公钥文件"#g' $file_name
perl -i -pe 's#\Q"What this CA is trusted to do"#"这个 CA 被信任做什么"#g' $file_name
perl -i -pe 's#\Q"Valid hosts this key is trusted to certify"#"此密钥可信任以证明的有效主机"#g' $file_name
